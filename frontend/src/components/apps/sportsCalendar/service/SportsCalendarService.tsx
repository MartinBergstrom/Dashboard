import axios from "axios";
import sportsCalendarData from "./../sportsCalendarData.json";
import {
  EventHighlight,
  SportsCalendarEvent,
} from "../model/SportsCalendarModels";

const BASE_URL = "http://localhost:5000";

export const getAllEvents = async (): Promise<SportsCalendarEvent[]> => {
  return new Promise<SportsCalendarEvent[]>((resolve) => {
    // Simulate a 5-second delay
    setTimeout(() => {
      const converted: SportsCalendarEvent[] = sportsCalendarData.map(
        (event) => ({
          ...event,
          pre_start_date: event.pre_start_date
            ? new Date(event.pre_start_date)
            : undefined,
          start_date: new Date(event.start_date),
          end_date: new Date(event.end_date),
          highlights: event.highlights
            ? convertHightlight(event.highlights)
            : [],
        })
      );
      resolve(converted);
    }, 2000);
  });

  //return sportsCalendarData;
  //const response = await axios.get(BASE_URL + "/sportsCalendar/events");
  //return response.data;
};

const convertHightlight = (
  highlights: { date: string; description: string; start_time?: string }[]
): EventHighlight[] => {
  return highlights.map((highlight) => ({
    ...highlight,
    date: new Date(highlight.date),
  }));
};
